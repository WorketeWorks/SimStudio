...
fn solve_gear_velocity(
    ...
    if perpendicular {
        ...
        solve_bevel_contact_impulse(gear, world, contact, tangent);
        return;
    }
    ...
    let denominator =
        jacobian_a * jacobian_a + jacobian_b * jacobian_b;
    ...
    if !fixed_a {
        world.bodies[gear.body_a].set_angvel(
            angular_a + axis_a * (lambda * jacobian_a),
            true,
        );
    }
    ...
    if !fixed_b {
        world.bodies[gear.body_b].set_angvel(
            angular_b + axis_b * (lambda * jacobian_b),
            true,
        );
    }
}
