...
type Connection={id:string;a:Piece;b:Piece;mode:JointMode;profile:ConnectionProfile;point:THREE.Vector3;axis:THREE.Vector3;travel:number;motorSpeed:number};
...
type AppState = {
  ...
  connectionModes:Map<string,{mode:JointMode;motorSpeed:number}>;
...
const allowedModes=(profile:ConnectionProfile):JointMode[]=>profile==="pin-round"?["fixed","rotation","motor"]:profile==="axle-cross"?["fixed","linear"]:["fixed","rotation","linear","rotation-linear","motor"];
const defaultMode=(profile:ConnectionProfile):JointMode=>profile==="pin-round"?"fixed":profile==="axle-cross"?"linear":"rotation-linear";
const modeLabel:Record<JointMode,string>={fixed:"Fija",rotation:"Rotación libre",linear:"Lineal libre","rotation-linear":"Rotación y lineal libres",motor:"Motor"};
...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    Object.assign(state,{scene,renderer,camera,floor,grid,pieces:[],connections:[],connectionModes:new Map<string,{mode:JointMode;motorSpeed:number}>(),running:false,addPart,preloadPart,debug:{colliders:false,connectors:false,physics:false},refreshDebug,updateDebug});appRef.current=state;
    ...
    const addConnection=(host:Piece,rod:Piece,socket:MeshConnector,shaft:MeshConnector)=>{
      ...
      const world=worldConnector(host,socket),id=`${host.id}:${rod.id}:${profile}`,saved=state.connectionModes.get(id),mode=saved&&allowedModes(profile).includes(saved.mode)?saved.mode:defaultMode(profile),motorSpeed=saved?.motorSpeed??3;state.connections.push({id,a:host,b:rod,mode,profile,point:world.point.clone(),axis:world.axis.clone(),travel:shaft.length??.5,motorSpeed});return true;
    };
  ...
  const reset=()=>{const s=appRef.current;if(!s)return;s.running=false;s.pieces.forEach(p=>{s.scene.remove(p.mesh);if(p.lockSprite)s.scene.remove(p.lockSprite)});s.pieces=[];s.connections=[];s.connectionModes.clear();s.snapshot=undefined;s.world=undefined;s.selected=undefined;s.refreshDebug();setRunning(false);setSelectedId(null);setCount(0)};
  const physics=async()=>{const s=appRef.current;if(!s)return;if(!s.running){
    ...
    s.connections.forEach(c=>{if(!c.a.body||!c.b.body)return;const a=c.point.clone().sub(c.a.mesh.position),b=c.point.clone().sub(c.b.mesh.position),axis=c.axis.clone().normalize();let joint:RAPIER.JointData;if(c.mode==="rotation"||c.mode==="motor")joint=RAPIER.JointData.revolute(a,b,axis);else if(c.mode==="linear")joint=RAPIER.JointData.prismatic(a,b,axis);else if(c.mode==="rotation-linear")joint=RAPIER.JointData.generic(a,b,axis,RAPIER.JointAxesMask.LinY|RAPIER.JointAxesMask.LinZ|RAPIER.JointAxesMask.AngY|RAPIER.JointAxesMask.AngZ);else joint=RAPIER.JointData.fixed(a,{x:0,y:0,z:0,w:1},b,{x:0,y:0,z:0,w:1});const created=world.createImpulseJoint(joint,c.a.body,c.b.body,true);created.setContactsEnabled(false);if(c.mode==="motor")(created as RAPIER.RevoluteImpulseJoint).configureMotorVelocity(c.motorSpeed,10);else if(c.mode==="rotation"&&c.b.frictionPin)(created as RAPIER.RevoluteImpulseJoint).configureMotorVelocity(0,3.5);if(c.mode==="linear"){const limit=Math.max(.15,c.travel/2);(created as RAPIER.PrismaticImpulseJoint).setLimits(-limit,limit)}});
  ...
  const setConnectionMode=(id:string,mode:JointMode)=>{const state=appRef.current,connection=state?.connections.find(item=>item.id===id);if(!state||!connection||running||!allowedModes(connection.profile).includes(mode))return;connection.mode=mode;state.connectionModes.set(id,{mode,motorSpeed:connection.motorSpeed});state.refreshDebug();setConnectionRevision(value=>value+1);setMessage(`${profileLabel[connection.profile]} · ${modeLabel[mode]}`)};
  const setMotorSpeed=(id:string,motorSpeed:number)=>{const state=appRef.current,connection=state?.connections.find(item=>item.id===id);if(!state||!connection||running)return;connection.motorSpeed=motorSpeed;state.connectionModes.set(id,{mode:connection.mode,motorSpeed});setConnectionRevision(value=>value+1);setMessage(`Motor ${motorSpeed.toFixed(1)} rad/s`)};
