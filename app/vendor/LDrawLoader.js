...
// Vendored from three r185. Sim Studio patch: when a primitive/subpart owns
// nested non-primitive children, upstream adds that child group without the
// parent type-1 matrix. Complex parts then repeat every nested child at the
// same origin. The small fix below carries that parent affine transform into
// the retained child hierarchy before it is rendered or serialized.
		...
		const processInfoSubobjects = async ( info, subobject = null ) => {
			...
			for ( let i = 0, l = subobjectInfos.length; i < l; i ++ ) {
				...
				if ( subobjectInfo.group.children.length ) {
					...
					subobjectInfo.group.applyMatrix4( subobject.matrix );
